...
## The app (laptop and phone)

`class-alarm run` serves one app that you use on both devices. It needs no App Store and no Mac.
...
- **Full-screen alarm:** when an alarm rings, the laptop's browser shows the message and a 4 digit code. Type it and press Enter to stop, or snooze. The terminal still works too.
- **Opens by itself** when `class-alarm run` starts and when an alarm rings (unless it's already open). Turn these off with `open_on_start` / `open_on_alarm`.
- **Test alarm now** button.
- The link to open on your phone.

The phone can see that the alarm is ringing but can't stop it, even with the key. Only requests from the laptop itself are allowed to stop or snooze, so you still have to get out of bed.

Setup:

1. In `config.toml` set `[web] enabled = true`.
2. Run `class-alarm run`. The laptop app opens in your browser, and the terminal prints a private phone link like `http://192.168.1.20:8765/?key=...`.
3. Open the phone link on your phone while it's on the **same Wi-Fi** as the laptop.
4. iPhone: Share > **Add to Home Screen**. Android: menu > **Add to Home screen**.
5. Lost the link? It's in the laptop app under "Open on your phone", or run `class-alarm web-link`.

Good to know:

- Keep the laptop app's tab open overnight if you like. It checks every 3 seconds, so the alarm screen appears right away. Sound always plays from the alarm program itself, even if the browser is closed.
- It only works while `class-alarm run` is running. Away from home the phone shows "Can't reach your laptop".
- The first time, Windows asks whether to allow Python through the firewall. Allow it on **private** networks.
- If your router gives the laptop a new address, the phone's Home Screen icon stops working. Either use the `.local` link that is also printed, or reserve the laptop's address in your router settings.
- The link contains a secret key. Anyone with the link on your Wi-Fi can use the app (but not stop alarms), so don't share it. To change the key, delete `data/web_key` and restart.
